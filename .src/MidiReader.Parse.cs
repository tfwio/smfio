...
namespace on.smfio
{
  public partial class MidiReader : IMidiParser_Parser
  {
    ...
    public byte[] GetMetaValue(int offset)
    {
      ...
      {
        /* 0xff00 */
        case MetaMsgU16FF.SequenceNo: return SmfFileHandle[SelectedTrackNumber, offset, 5];
        /* 0xff20 */
        case MetaMsgU16FF.Chanel: return SmfFileHandle[SelectedTrackNumber, offset, 4];
        /* 0xff51 */
        case MetaMsgU16FF.Tempo: return SmfFileHandle[SelectedTrackNumber, offset, 6];
        /* 0xff54 */
        case MetaMsgU16FF.SMPTE: return SmfFileHandle[SelectedTrackNumber, offset, 4];
        /* 0xff58 */
        case MetaMsgU16FF.TimeSignature: return SmfFileHandle[SelectedTrackNumber, offset, 7];
        /* 0xff59 */
        case MetaMsgU16FF.KeySignature: return SmfFileHandle[SelectedTrackNumber, offset, 5];
        /* 0xff2f */
        case MetaMsgU16FF.EndOfTrack: return SmfFileHandle[SelectedTrackNumber, offset, 2];
        /* 0xff7f */
        case MetaMsgU16FF.SystemSpecific: return SmfFileHandle[SelectedTrackNumber, offset, 3];
        /* 0xff7f */
        case MetaMsgU16FF.SystemExclusive: return SmfFileHandle[SelectedTrackNumber, offset, 4];
        default:
    ...
    string GetEventString(int offset, int plus)
    {
      ...
      if (!MidiMessageInfo.IsMidiBMessage(CurrentTrackRunningStatus))
      {
        ...
        return GetNoteMsg(0, offset + plus, StringRes.m9);
      else if (MidiMessageInfo.IsNoteOff(ExpandedRSE))
        return GetNoteMsg(0, offset + plus, StringRes.m8);
      else if (MidiMessageInfo.IsKeyAftertouch(ExpandedRSE))
        return string.Format(StringRes.mA, SmfFileHandle[SelectedTrackNumber, offset + plus], SmfFileHandle[SelectedTrackNumber, offset + plus + 1]);
      else if (MidiMessageInfo.IsControlChange(ExpandedRSE))
        return string.Format(StringRes.mB, SmfString.ControlMap[SmfFileHandle.Get8Bit(SelectedTrackNumber, offset + plus)].Replace((char)0xa, (char)0x20).Trim(), SmfFileHandle.Get8Bit(SelectedTrackNumber, offset + plus + 1));
      else if (MidiMessageInfo.IsProgramChange(ExpandedRSE))
        return SmfString.PatchMap[SmfFileHandle[SelectedTrackNumber, offset + plus]].Replace((char)0xa, (char)0x20).Trim();
